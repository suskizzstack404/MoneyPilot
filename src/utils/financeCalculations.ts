...
/**
 * All-time totals (income/expenses/net/count) for the given transaction set,
 * paired with month-over-month percentage deltas so summary cards can show
 * both "how much" and "trending up or down".
 */
export function computeTransactionSummary(transactions: Transaction[]): TransactionSummary {
  ...
  return {
    ...
    totalCountChange: percentChange(thisMonthCount, lastMonthCount),
    totalIncome,
    totalIncomeChange: percentChange(thisMonthIncome, lastMonthIncome),
    totalExpenses,
    totalExpensesChange: percentChange(thisMonthExpenses, lastMonthExpenses),
    netCashFlow,
    netCashFlowChange: percentChange(thisMonthNet, lastMonthNet),
  }
}
