...
export default function Sidebar({ collapsed, onToggleCollapse, mobileOpen, onCloseMobile }: SidebarProps) {
  ...
  const content = (
    <div className="flex flex-col h-full">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `group relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-200 ${
                isActive
                  ? 'text-white bg-mint-emerald shadow-glow-sm'
                  : 'text-ink-300 hover:text-ink-100 hover:bg-hoverTint'
      ...
      <div className="px-3 pb-5 flex flex-col gap-1 border-t border-slate-200 pt-3 mt-2">
        <NavLink
          to="/dashboard/settings"
          onClick={onCloseMobile}
          className={({ isActive }) =>
            `relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-200 ${
              isActive ? 'text-ink-100 bg-slate-100' : 'text-ink-300 hover:text-ink-100 hover:bg-slate-50'
            } ${collapsed ? 'justify-center px-0' : ''}`
          }
        ...
        <button
          ...
          className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-ink-300 hover:text-rose-500 hover:bg-rose-500/[0.06] transition-all duration-200 ${
            collapsed ? 'justify-center px-0' : ''
          }`}
        ...
        <button
          ...
          className="hidden lg:flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-ink-500 hover:text-ink-100 hover:bg-slate-50 transition-all duration-200 mt-1"
        >
          {collapsed ? (
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className="hidden lg:block flex-shrink-0 h-screen sticky top-0 glass border-r border-slate-200 overflow-hidden"
      >
        {content}
      </motion.aside>
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              className="lg:hidden fixed inset-y-0 left-0 w-[260px] glass border-r border-slate-200 z-50"
            >
              <button
                onClick={onCloseMobile}
